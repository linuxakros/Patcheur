...
static int generate_patch(const wchar_t* oldPath, const wchar_t* newPath,
    ...
    // Patch format: magic, version, compression flag, original/new sizes,
    // checksum, then payload. The generator intentionally stores the complete
    // target image so that application is deterministic.
